...
struct lsquic_stream_ctx {
    ...
    DIR                 *req_dir;
    unsigned             change_file_wait_times;
    lsquic_time_t        last_file_sent_time;
...
bytes_left (lsquic_stream_ctx_t *st_h)
{
    ...
    {
        size_t remained_size = test_reader_size(st_h->reader.lsqr_ctx);
        if (st_h->req_dir && remained_size <= 0)
        {
            LSQ_DEBUG("FCT DEBUG: %s transfer cost %"PRIu64" microseconds", st_h->req_filename, lsquic_time_now()-st_h->last_file_sent_time);
            st_h->change_file_wait_times += 1;
            struct dirent *tempfile;
            char *filename;
    nextfile:
            tempfile = readdir(st_h->req_dir);
            if (tempfile)
            {
                if (!strcmp(tempfile->d_name, ".") || !strcmp(tempfile->d_name, ".."))
                {
                    LSQ_DEBUG("%s is a dir, seek next file.", tempfile->d_name);
                    goto nextfile;
                }
                filename = malloc(strlen(st_h->req_path) + 60);
                strcpy(filename, st_h->req_path);
                strcat(filename, "/");
                strcat(filename, tempfile->d_name);
                LSQ_DEBUG("[%s] is a dir, next file is [%s]", st_h->req_path, filename);
...
http_server_on_write (lsquic_stream_t *stream, lsquic_stream_ctx_t *st_h)
{
    if (st_h->flags & SH_HEADERS_SENT)
    {
        ...
        {
            if (st_h->change_file_wait_times)
            {
                st_h->change_file_wait_times = 0;
                st_h->last_file_sent_time = lsquic_time_now();
            }
...
process_request (struct lsquic_stream *stream, lsquic_stream_ctx_t *st_h)
{
    ...
    else
    {
        ...
        struct stat s_buf;
        stat(st_h->req_path, &s_buf);
        if (S_ISDIR(s_buf.st_mode))
        {
            struct dirent *tempfile;
            char *filename;
            DIR *dp = opendir(st_h->req_path);
            tempfile = readdir(dp);
            filename = malloc(strlen(st_h->req_path) + 60);
            strcpy(filename, st_h->req_path);
            strcat(filename, "/");
            strcat(filename, tempfile->d_name);
            LSQ_DEBUG("[%s] is a dir, first file is [%s]", st_h->req_path, filename);
            st_h->req_dir = dp;
            st_h->req_filename = filename;
            st_h->reader.lsqr_ctx = create_lsquic_reader_ctx(filename);
            st_h->last_file_sent_time = lsquic_time_now();
...
{
    lsquic_stream_record_fct(stream, st_h->req_filename);
    free(st_h->req_filename);
    free(st_h->req_path);
    if (st_h->reader.lsqr_ctx)
        destroy_lsquic_reader_ctx(st_h->reader.lsqr_ctx);
