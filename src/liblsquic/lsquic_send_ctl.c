...
static void
send_ctl_dup_append (struct lsquic_send_ctl *ctl,
                       struct lsquic_packet_out *packet_out)
{
    LSQ_DEBUG("append duplicate packets %"PRIu64, packet_out->po_packno);
    TAILQ_INSERT_TAIL(&ctl->sc_duplicate_packets, packet_out, po_next);
}
                      ...
                      struct lsquic_packet_out *packet_out)
{
    ...
    ++ctl->sc_n_scheduled;
    LSQ_DEBUG("after append schedule packets %u", ctl->sc_n_scheduled);
...
update_for_resending (lsquic_send_ctl_t *ctl, lsquic_packet_out_t *packet_out)
{
    ...
    LSQ_DEBUG("Resend packets frame type %u", packet_out->po_frame_types);
...
struct lsquic_packet_out*
lsquic_send_ctl_copy_packet_out (struct lsquic_send_ctl *ctl, struct lsquic_packet_out *old_packet_out, unsigned take_pno, unsigned retrans_times)
{
    struct lsquic_conn *const lconn = ctl->sc_conn_pub->lconn;
    struct lsquic_packet_out *new;
    struct packet_resize_ctx prctx;
    struct resize_one_packet_ctx one_ctx = {
                    ctl, old_packet_out, old_packet_out->po_path,
                    lsquic_packet_out_pns(old_packet_out), 0, 0,
    };
    LSQ_DEBUG("origin po_loss_chain is %"PRIu64, old_packet_out->po_loss_chain->po_packno);
    lsquic_packet_resize_init(&prctx, ctl->sc_enpub, lconn, &one_ctx,
                                                        &resize_one_funcs);
    new = lsquic_packet_resize_next(&prctx);
    new->po_flags &= ~PO_SENT_SZ;
    new->po_retrans_times = retrans_times;
    new->po_retrans_no = old_packet_out->po_retrans_no != 0 ? old_packet_out->po_retrans_no : old_packet_out->po_packno;
    if (take_pno)
    {
